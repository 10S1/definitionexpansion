...
if __name__ == '__main__':
    ...
    path = Path(__file__).parent.parent / 'sHTML' / 'Overhead' / 'All examples' / 'statementE000-firstPart.en.xhtml'
    shtml = parse_shtml(path)
    xs, string = get_gfxml_string(shtml)
    sentence = sentence_tokenize(string)[0]
    sentence = sentence[0].lower() + sentence[1:]
    print(sentence)
    gf_ast = shell.handle_command(f'p -cat=Stmt "{sentence[:-1]}"')
    print(gf_ast)
    tree = build_tree(xs, gf_ast)
    print(tree)
    make_graph_files(tree)
